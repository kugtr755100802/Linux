...
manager构造函数：在创建manager时，初始化内核表，逻辑服务器，然后与
                 ...
                 表示已经有一个准备就绪的连接等待处理
*/
mgr::mgr( int epollfd, const host& srv ) : m_logic_srv( srv )//冒号是用来初始化的
{
    //初始化内核事件表
    m_epollfd = epollfd;
    int ret = 0;
...
{
    int cltfd = connection->m_cltfd;//客户端fd
    int srvfd = connection->m_srvfd;//服务端fd
...
RET_CODE mgr::process( int fd, OP_TYPE type )
{
    ...
    else if( connection->m_srvfd == fd )
    {
        ...
        switch( type )
        {
            ...
            case WRITE:
            {
                RET_CODE res = connection->write_srv();//写入服务端
                switch( res )
                {
                    case TRY_AGAIN:
                    {
